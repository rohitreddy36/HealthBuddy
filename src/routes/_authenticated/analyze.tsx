...
function Analyze() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <header>
        <h1 className="font-display text-2xl font-semibold">Symptom check</h1>
      ...
      {stage === "input" && (
        <div className="rounded-2xl border bg-card p-6 space-y-4">
          ...
          <Button onClick={next} disabled={loading || !form.symptoms.trim()} className="w-full">
            {loading ? <><Loader2 className="size-4 animate-spin" /> Preparing questions…</> : "Continue"}
          </Button>
        </div>
      )}

      {stage === "follow" && followUps && (
        <div className="rounded-2xl border bg-card p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {followUps.follow_up_symptoms.map((s) => (
              <label
                key={s}
                className="flex items-center gap-3 rounded-lg border p-3 cursor-pointer hover:bg-secondary/50"
              >
                <Checkbox
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={analyze} disabled={loading} className="flex-1">
              {loading ? <><Loader2 className="size-4 animate-spin" /> Analyzing…</> : "Get guidance"}
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <Section title="Self-care guidance">
            <ul className="text-sm space-y-1 list-disc pl-5">
              {result.self_care.map((s) => <li key={s}>{s}</li>)}
            </ul>
          </Section>
          ...
          <Section title="Diet recommendations">
            <div className="grid md:grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <ul className="list-disc pl-5">{result.diet.include.map((s) => <li key={s}>{s}</li>)}</ul>
              </div>
              <div>
                <div className="font-medium text-destructive">Avoid</div>
                <ul className="list-disc pl-5">{result.diet.avoid.map((s) => <li key={s}>{s}</li>)}</ul>
              </div>
            </div>
            <div className="mt-3 text-sm">
              <div className="font-medium mb-1">Sample meal plan</div>
              <ul className="space-y-1">
                <li><b>Breakfast:</b> {result.diet.sample_meal_plan.breakfast}</li>
                <li><b>Lunch:</b> {result.diet.sample_meal_plan.lunch}</li>
                <li><b>Dinner:</b> {result.diet.sample_meal_plan.dinner}</li>
                <li><b>Snacks:</b> {result.diet.sample_meal_plan.snacks}</li>
              </ul>
          ...
          <Section title="Exercise & recovery">
            <ul className="text-sm space-y-1 list-disc pl-5">
              {result.exercise.map((s) => <li key={s}>{s}</li>)}
            </ul>
          </Section>

          <Section title="When to seek medical attention" icon={<AlertCircle className="size-4 text-primary" />}>
